...
export class SupervisorComponent implements OnInit {
  ...
  cargarUsuarios() {
    ...
    this.empleados$ = collectionData(empleadosRef, {
      idField: 'id',
    }) as Observable<Usuario[]>;
    const clientesRef = collection(this.firestore, 'clientes');
    this.clientes$ = collectionData(clientesRef, {
      idField: 'id',
    }) as Observable<Usuario[]>;
  }
  ...
  async submitEvaluacion(event: Event) {
    ...
    try {
      ...
      const evaluacionesCollection = collection(
        this.firestore,
        'encuestas_supervisores'
      );
      ...
      this.notificationService.showError(
        'Error al guardar la evaluación. Inténtelo de nuevo.',
        'Error'
      );
  ...
  private generarGraficos(data: Evaluacion[]) {
    ...
    if (data.length === 0) {
      console.warn(
        'No hay datos de calificaciones para mostrar en el historial.'
      );
    ...
    setTimeout(() => {
      try {
        const calificacionPromedio =
          data.reduce((acc, curr) => acc + curr.calificacion, 0) / data.length;
        ...
        const areasCount = areas.map(
          (area) => data.filter((e) => e.areasEvaluadas.includes(area)).length
        );
        ...
        console.error(
          'Error al generar el gráfico de historial de calificaciones:',
          error
        );
        this.notificationService.showError(
          'Error al generar el gráfico de historial de calificaciones',
